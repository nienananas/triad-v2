...
class LibESTBitermExtractionTest {
    ...
    void testCSourceBitermExtraction() {
        ...
        Map<String, Integer> expectedBiterms = Map.ofEntries(
            ...
            Map.entry("certifSet", 2), Map.entry("certifSsl", 1), Map.entry("certifUse", 1),
            Map.entry("clientEst", 2), Map.entry("clientKey", 2), Map.entry("clientSet", 2),
            Map.entry("contextSsl", 2), Map.entry("contextUse", 2), Map.entry("ctxSsl", 2),
            Map.entry("ctxUse", 1), Map.entry("errLog", 1), Map.entry("estKey", 2),
            Map.entry("estSet", 2), Map.entry("evpPkey", 1), Map.entry("functionSet", 2),
            Map.entry("functionUtil", 2), Map.entry("keySet", 4), Map.entry("privatSet", 2),
            Map.entry("setUse", 2), Map.entry("sslUse", 1)
        );
    ...
    void testCTestCodeBitermExtraction() {
        ...
        // Ground truth from original TRIAD output
        Map<String, Integer> expectedBiterms = Map.ofEntries(
                Map.entry("bnNew", 1), Map.entry("bnSet", 1),
                Map.entry("bnWord", 1), Map.entry("evpPkey", 1), Map.entry("exGenerat", 1),
                Map.entry("exKey", 1), Map.entry("exRsa", 1), Map.entry("generatKey", 1),
                Map.entry("generatRsa", 1), Map.entry("keyRsa", 1), Map.entry("newRsa", 1),
                Map.entry("setWord", 1)
        );
